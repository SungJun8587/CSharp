...
{
    /// <summary>MySqlBulk 유틸 함수</summary>
    internal class MySqlBulkCommon
    {
        /// <summary>
        ...
        /// <param name="sqlBulkCopy">MySql 데이터베이스 대량 데이터 처리 객체</param>
        /// <returns>데이터 테이블(DataTable)</returns>
        internal static DataTable GetDataTableFromFields<T>(IEnumerable<T> data, MySqlBulkCopy sqlBulkCopy)
        {
            int i = 0;
            ...
            foreach (var propertyInfo in listType)
            {
                ...
                sqlBulkCopy.ColumnMappings.Add(new MySqlBulkCopyColumnMapping(i++, column.ColumnName));
        ...
        /// <returns>데이터 테이블(DataTable)</returns>
        internal static DataTable GetDataTableFromFields<T>(IEnumerable<T> data, MySqlBulkCopy sqlBulkCopy, List<string> columnNames)
        {
            var listType = typeof(T).GetProperties();
            var list = new List<PropertiesModel>();
            foreach (var propertyInfo in listType)
