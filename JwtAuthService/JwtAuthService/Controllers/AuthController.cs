...
namespace JwtAuthService.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        {
            // 1. 로그인 시도
            var result = await _authService.LoginAsync(request.Username, request.Password, request.DeviceId);
        ...
        {
            // 1. 리프레시 토큰 갱신
            var result = await _authService.RefreshAsync(request.RefreshToken, request.DeviceId);

            // 2. 갱신 실패 시
            if (result == null) return BadRequest(new { error = "invalid_or_expired" });
